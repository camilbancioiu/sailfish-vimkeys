...
*/
		...
		TextSwitch {
		    automaticCheck: false
		    checked: (emojiKeyboard.sendStatistics) ? true : false
		    text: "Send usage statistics"
		    description: "Allow statistics collection for anonymous usage analytics. More on privacy policy below."
		    onClicked: {
			emojiKeyboard.sendStatistics = (emojiKeyboard.sendStatistics > 0) ? 0 : 1;
			emojiKeyboard.saveSetting('Statistics',emojiKeyboard.sendStatistics);
		    }
		}
		...
		Label {
		    ...
		    wrapMode: Text.WrapAtWordBoundaryOrAnywhere
		    text: "The Emoji Keyboard collects anonymous statistics from keyboard usage by sending information over the internet connection to Google analytics service. Statistics collection sends following attributes:\n· anonymous unique device identifier\n· keyboard launch\n· opened symbol set and page\n· Geo location based on IP address\n\nStatistics information will be collected unless user opt-out by deselecting statistics collection from settings. Statistics will be used to improve The Emoji Keyboard and to get Geo location on country level to know where keyboard is in use.\nAuthor of The Emoji Keyboard will not hand over collected data to any third parties except Google as service provider hosting analytics service. Google may combine data to other data collected in their services but device identity has been anonymized to prevent data aggregation with analytics data collected from other applications and service. User of The Emoji Keyboard can always disable Usage Statistics collection."
		}
