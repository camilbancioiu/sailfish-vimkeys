...
KeyboardLayout {
	...
	// permission to send statistics
	sendStatistics = getSetting('Statistics',sendStatistics);
	console.log("emojiKeyboard.sendStatistics",sendStatistics);
	// Send statistics only after reading configuration to be able to follow privacy settings
	emojiStatistics.statLoaded(emojiSet,emojiPage);
    }
...
//	width: keyboard.width
	FunctionKey {
		...
		emojiStatistics.statPage(emojiSet,emojiPage);
		canvas.updateIMArea()
	    }
	}
    ...
    EmojiConfigRow { }
    EmojiStatistics { id: emojiStatistics }
